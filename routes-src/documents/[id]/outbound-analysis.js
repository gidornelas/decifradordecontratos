...
module.exports = async function handler(req, res) {
  ...
  var requestContext = observability.logRequestStart(req, res, {
    route: "documents.outboundAnalysis"
  });
  ...
  try {
    ...
    var result = await outboundAnalysis.getLatestOutboundAnalysisForDocument(
      documentId,
      currentUserId
    );
    ...
    if (!result) {
      ...
      return http.badRequest(res, "No outbound analysis found for this document.");
    }
    ...
    observability.logAppError("documents.outbound_analysis_failed", error, {
      route: "documents.outboundAnalysis",
      requestId: requestContext.requestId,
      userId: currentUserId
    });
