...
async function createAnalysisForDocument(input) {
  ...
  var pendingAnalysis = await db.query(
    [
      ...
      "document_id, source_document_id, user_id, status, prompt_version, analysis_kind, analysis_perspective",
      ") values ($1, $1, $2, $3, $4, $5, $6)",
      "returning id"
    ].join(" "),
    [documentId, userId, "pending", "contract-analysis-v1", ANALYSIS_KIND_RECEIVED, ANALYSIS_PERSPECTIVE_RECEIVER]
  );
...
async function getAnalysisById(analysisId, userId) {
  var analysisResult = await db.query(
    [
      ...
      "a.analysis_kind, a.analysis_perspective, a.source_document_id, a.reference_document_id,",
      "a.final_verdict, a.proposal_consistency_score, a.send_readiness_score,",
      ...
      "left join documents source_doc on source_doc.id = a.source_document_id",
      "left join documents reference_doc on reference_doc.id = a.reference_document_id",
...
async function getRecentAnalysisHealthSummary(options) {
  ...
  var recentResult = await db.query(
    [
      ...
      "from analyses",
      "where coalesce(analysis_kind, $2) = $2",
      "and created_at >= timezone('utc', now()) - ($1::text || ' hours')::interval"
    ].join(" "),
    [String(windowHours), ANALYSIS_KIND_RECEIVED]
  );
  ...
  var latestFailureResult = await db.query(
    [
      ...
      "from analyses a",
      "join documents d on d.id = a.document_id",
      "where a.status = 'failed' and coalesce(a.analysis_kind, $1) = $1",
