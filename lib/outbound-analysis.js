...
async function getOutboundAnalysisById(analysisId, userId) {
  if (!(await supportsOutboundSchema())) {
    return null;
  }
...
async function supportsOutboundSchema() {
  ...
  return Boolean(
    ...
    capabilities.hasAnalysisIssuesTable
  );
}

async function ensureOutboundSchema() {
  if (!(await supportsOutboundSchema())) {
    throw new Error("Outbound validation is not available until the database migration is applied.");
  }
}
